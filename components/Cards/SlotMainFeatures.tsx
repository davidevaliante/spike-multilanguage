    ...
    return (
        <Container style={style}>
            ...
            <div style={{ padding: "1rem" }}>
                ...
                <StarsContainer>
                    <p style={{ marginRight: ".5rem", fontSize: "1rem" }} className="desc">
                        {t("Volatility")}
                    </p>
                    <div style={{ display: "flex" }}>
                        {[...Array(volatilityToNumberOfStars()).keys()].map((s, i) => (
                            <img
                                key={`volatility_${i}_start_full`}
                                alt="full_star_icon"
                                className="star"
                                src="/icons/star_full.svg"
                            />
                        ))}
                        {[...Array(5 - volatilityToNumberOfStars()).keys()].map((s, i) => (
                            <img
                                key={`volatility_${i}_start_empty`}
                                alt="empty_star_icon"
                                className="star"
                ...
                <StarsContainer>
                    <p style={{ marginRight: ".5rem", fontSize: "1rem" }} className="desc">
                        {t("Winning Spin Frequency")}
                    </p>
                    <div style={{ display: "flex" }}>
                        {winningSpinFrequency ? (
                            <>
                                {[...Array(winningSpinFrequency).keys()].map((s, i) => (
                                    <img
                                        key={`volatility_${i}_start_full`}
                                        alt="full_star_icon"
                                        className="star"
                                ...
                                {[...Array(5 - winningSpinFrequency).keys()].map((s, i) => (
                                    <img
                                        key={`volatility_${i}_start_empty`}
                                        alt="empty_star_icon"
                                        className="star"
                ...
                <SuggestedCasino>
                    <a rel="nofollow" href={`/go?to=${bonusLink}`}>
                        <h3>{t("VISIT THE SITE")}</h3>
                        <h4>{bonusName}</h4>
                    </a>
                </SuggestedCasino>

                <PaymentAccepetedIcons>{mapPaymentsMethodsToIcons()}</PaymentAccepetedIcons>
            </div>
        </Container>
    )
}
