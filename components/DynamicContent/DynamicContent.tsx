...
const DynamicContent: FunctionComponent<DynamicContentProps> = ({ content, isBakeca }) => {
    ...
    const bonusListRenderer: FunctionComponent<DynamicBonusList> = ({ bonus, direction, tableLabel, collapsable }) => {
        const remapBonusLink = () =>
            bonus.map((b) => {
                if (b.bonus.name === 'LeoVegas')
                    b.bonus.link = 'https://ads.leovegas.com/redirect.aspx?pid=3704891&bid=1496'
                ...
                if (b.bonus.name === 'Starcasinò')
                    b.bonus.link = 'http://record.affiliatelounge.com/_SEA3QA6bJTMP_fzV1idzxmNd7ZgqdRLk/132/'
                if (b.bonus.name === 'GoldBet')
                    b.bonus.link = 'https://media.goldbetpartners.it/redirect.aspx?pid=4641&bid=1495'
                if (b.bonus.name === 'Starvegas')
                    b.bonus.link = 'https://www.starvegas.it/gmg/refer/60a2b6ffcb4f5e0001afa975'
                if (b.bonus.name === 'Eurobet')
                    b.bonus.link = 'https://record.betpartners.it/_E_C7XwxgprAZV93hC2dJ_GNd7ZgqdRLk/110/'
                if (b.bonus.name === 'Gioco Digitale')
                    b.bonus.link = 'https://mediaserver.entainpartners.com/renderBanner.do?zoneId=2022788'
                return b
            })
        ...
        if (!collapsed && direction === 'vertical')
            return (
                <div key={bonus[0].bonus.id} style={{ margin: '0rem 0rem' }}>
                    ...
                    {isBakeca
                        ? remapBonusLink().map((b) => <BonusStripe key={b.bonus.name} bonus={b.bonus} />)
                        : bonus.map((b) => <BonusStripe key={b.bonus.name} bonus={b.bonus} />)}
                    {collapsable && (
                        <LoadMoreButtonStyle onClick={() => setCollapsed(!collapsed)}>Nascondi</LoadMoreButtonStyle>
                    )}
                </div>
            )
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap' }}>
                        {isBakeca
                            ? remapBonusLink().map((b) => (
                                  <PrimaryBonusCard withSuggestion={false} key={b.bonus.name} bonus={b.bonus} />
                              ))
                            : bonus.map((b) => (
                                  <PrimaryBonusCard withSuggestion={false} key={b.bonus.name} bonus={b.bonus} />
                              ))}
        ...
        return <LoadMoreButtonStyle onClick={() => setCollapsed(!collapsed)}>Mostra altri</LoadMoreButtonStyle>
    }
