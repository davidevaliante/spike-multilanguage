...
interface TableHeadProps {
    ...
    numSelected: number;
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof DreamCatcherSpin) => void;
    order: Order;
    orderBy: string;
    rowCount: number;
}
...
export const EnhancedTableHead : FunctionComponent<TableHeadProps> = ({ classes,  order, orderBy, numSelected, rowCount, onRequestSort }) => {
    ...
    const createSortHandler = (property: keyof DreamCatcherSpin) => (event: React.MouseEvent<unknown>) => {
        ...
        <TableHead style={{background : '#db0d30'}}>
            <TableRow>
                {headCells.map((headCell, i) => (
                    headCell.id === 'spinResultSymbol' ? <StyledTableCell key={headCell.id}>{t(headCell.label)}</StyledTableCell> :
                    <StyledTableCell
                        key={headCell.id}
...
export const DreamCatcherTable : FunctionComponent<EnhancedTableProps> = ({rows}) => {

    const classes = useStyles()
    const [order, setOrder] = useState<Order>('desc')
    const [orderBy, setOrderBy] = useState<keyof DreamCatcherSpin>('date')
    ...
    const handleRequestSort = (event: React.MouseEvent<unknown>, property: keyof DreamCatcherSpin) => {
                                ...
                                return (
                                    <StyledTableRow
                                        ...
                                        <TableCell align='left'>
                                            <SpinResultSpan>
                                                {symbolToSpinResultImage(row.spinResultSymbol as string, 'dreamcatcher')}
                                            </SpinResultSpan>
                                        </TableCell>
                                        <TableCell align='left'>
                                            {row.multiplier}
