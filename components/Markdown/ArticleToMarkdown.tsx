...
const ArticleToMarkdown: FunctionComponent<Props> = ({
  ...
  const replaceWithCustomElement = (props: any) => {
    ...
    if (elementType === "spikeVideo") {
      ...
      const videoLink =
        props.children[0].props.children[0].props.children.split("@")[1];
      return (
    ...
    if (elementType === "imageButton") {
      ...
      const imageLink = imageButtonData
        .find((d) => d.includes("img="))
        ?.split("img=")[1];
      const link = imageButtonData
        .find((d) => d.includes("link="))
        ?.split("link=")[1];
