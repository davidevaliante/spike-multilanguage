            ...
            <Backdrop style={{zIndex : 10}} open={showSpamBonuses}>
                    <DesktopContainer>
                        <Paper ref={bonusRef} elevation={3} style={{padding : '2rem'}}>
                            <div style={{display : 'flex', justifyContent : 'space-between', alignItems : 'space-around'}}>
                                <h1 style={{marginLeft : '1rem', fontWeight : 'bold', fontSize : '1.5rem', color : 'crimson', marginBottom : '2rem' }}>{t('Compare legal websites')}</h1>
                                <img onClick={() => handleCloseSpamBonuses()} style={{width : '36px', height : '36px', cursor : 'pointer'}} src='/icons/close_red.svg'/>
                            </div>
                            <div style={{display : 'flex', justifyContent : 'space-between'}}>
                                {[...bonuses.slice(0,3)].map(b => <PrimaryBonusCard key={b.name} style={{margin : '1rem'}} bonus={b}/>)}
                            </div>
                        </Paper>
                    </DesktopContainer>
                    ...
                    <MobileContainer>
                        <Paper ref={bonusRef} elevation={3} style={{padding : '2rem'}}>
                            ...
                            <div style={{}}>
                                {[...bonuses.slice(0,3)].map(b => <SecondaryBonusCard key={b.name} bonus={b}/>)}
                            </div>
                        </Paper>
                    </MobileContainer>
            </Backdrop>
