...
const Index: FunctionComponent<PageProps> = ({ _shallow = false, _home }) => {
  ...
  const [isBakeca, setIsBakeca] = useState(
    router.asPath.split("from=")[1] === "bakeca"
  );

  const [loading, setLoading] = useState(true);
  const [home, setHome] = useState<Home>(_home);
  const [producerSlots, setProducerSlots] = useState<ApolloSlotCard[]>(
    _home.producerSlots.slot.map((s) => s.slot)
  );
  const [onlineSlots, setOnlineSlots] = useState<ApolloSlotCard[]>(
    _home.onlineSlots.slot.map((s) => s.slot)
  );
  const [barSlots, setBarSlots] = useState<ApolloSlotCard[]>(
    _home.barSlots.slot.map((s) => s.slot)
  );
  const [vltSlots, setVltSlots] = useState<ApolloSlotCard[]>(
    _home.vltSlots.slot.map((s) => s.slot)
  );
  const [bonusList, setBonusList] = useState<ApolloBonusCardReveal[]>(
    _home.bonuses.bonus.map((b) => b.bonus)
  );
  ...
  return (
    <div>
      ...
      <NavbarProvider
        currentPage={!isBakeca ? "Home" : "/bakeca-home"}
        countryCode={contextCountry}
      >
        {home.topArticle && (
          <HomeHeader topArticle={home.topArticle}>SPIKE SLOT</HomeHeader>
        )}
        <BodyContainer>
          <Logo />
          {userCountryEquivalentExists && (
            <CountryEquivalentPageSnackbar path={`/${userCountry}`} />
          )}
          <MainColumn>
            <LiveStatsCta />

            {producerSlots && (
              <HighlightProducerSlideShow producerSlots={producerSlots} />
            )}

            <LazyLoad height={450} once>
              <SlideShow
                apolloSlotCards={onlineSlots.filter(
                  (s) => s.image !== undefined
                )}
            ...
            {barSlots && (
              <LazyLoad height={450} once offset={100}>
                <SlideShow
                  apolloSlotCards={barSlots?.filter(
                    (s) => s.image !== undefined
                  )}
            ...
            {vltSlots && (
              <LazyLoad height={450} once offset={100}>
                <SlideShow
                  apolloSlotCards={vltSlots?.filter(
                    (s) => s.image !== undefined
                  )}
          ...
          <RightColumn>
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              <h4 className="video-header">
                {t(`Watch SPIKE's latest video`)}
              </h4>
            </div>
