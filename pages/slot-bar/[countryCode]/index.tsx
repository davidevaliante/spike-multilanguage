...
const Slots: FunctionComponent<Props> = ({
  ...
  const [initialSlots, setInitialSlots] = useState<
    AlgoliaSearchResult[] | undefined
  >(_initialSlots);
  const [bonusList, setBonusList] =
    useState<{ bonus: ApolloBonusCardReveal }[]>(_bonusList);
  const [slotListArticles, setSlotListArticles] =
    useState<SlotListArticles>(_slotListArticles);
  const [highlightSlot, setHighlightSlot] =
    useState<ApolloSlotCard>(_highlightSlot);
  const [barSlotListPage, setBarSlotListPage] = useState<
    BarSlotListPage | undefined
  >(_barSlotListPage[0]);
  ...
  const [slotList, setSlotList] = useState<AlgoliaSearchResult[] | undefined>(
    _initialSlots
  );

  const [userCountryEquivalentExists, setUserCountryEquivalentExists] =
    useState(false);
  ...
  const [searchTimerId, setSearchTimerId] = useState<number | undefined>(
    undefined
  );
  ...
  // ordinamento
  const [ordering, setOrdering] = useState<"date" | "alphabetical" | "rating">(
    "date"
  );

  useEffect(() => {
    if (ordering === "alphabetical")
      setSlotList(orderBy(initialSlots, ["name"], ["asc"]));
    if (ordering === "rating")
      setSlotList(orderBy(initialSlots, ["rating"], ["desc"]));
    if (ordering === "date")
      setSlotList(orderBy(initialSlots, ["created_at"], ["desc"]));
  }, [ordering]);
  ...
  const loadNextOrderedBatch = async (
    ...
    setSlotList([
      ...slotList!,
      ...(response.data.data.slots as AlgoliaSearchResult[]),
    ]);
  };
    ...
    <StyleProvider>
      <Head>
        ...
        <meta
          ...
          content={buildContentLanguageString(contextCountry)}
        ></meta>
        ...
        <meta
          itemProp="image"
          content={"https://spikewebsitemedia.b-cdn.net/spike_share_img.jpg"}
        />
        ...
        <BodyContainer>
          <MainColumn>
            ...
            <ArticleToMarkdown
              ...
              content={injectCDN(barSlotListPage?.topArticle!)}
            />

            {barSlotListPage?.sliderSlots[0].slot && (
              <LazyLoad height={450} once offset={100}>
                <SlideShow
                  apolloSlotCards={barSlotListPage?.sliderSlots[0].slot.map(
                    (s) => s.slot
                  )}
                    ...
                    {showMoreFilter && (
                      ...
                      >
                        ...
                        <h4
                          ...
                          onClick={() =>
                            showProducersOrCategories("categories")
                          }
                        >
                          {t("Categories")}
                        </h4>
            ...
            <SlotList
              slotList={slotList}
              showSearchHasNoResults={showSearchHasNoResults}
            />

            {/* <LoadMoreButton onLoadMore={() => loadNextOrderedBatch(ordering)} /> */}

            {barSlotListPage?.bottomArticle && (
              <ArticleToMarkdown
                style={{ padding: "0rem 1rem" }}
                content={injectCDN(barSlotListPage?.bottomArticle!)}
              />
            )}
          </MainColumn>
          ...
          <RightColumn>
            ...
            <div style={{ top: "2rem" }} className="bonus-column-container">
              {bonusList &&
                bonusList.map((bo) => (
                  <ApolloBonusCardRevealComponent
                    key={bo.bonus.name}
                    bonus={bo.bonus}
                  />
                ))}
...
export async function getServerSideProps({ query, req, res }) {
  ...
  const slotListArticles =
    slotListArticlesResponse.data.data.slotListArticles[0];
  const barSlotListPage = barSlotListResponse.data.data.barSlotLists;

  if (somethingIsUndefined([slotList, bonusList, slotListArticles]))
    serverSideRedirect(res, "/slot-bar/row");
  return {
    props: {
