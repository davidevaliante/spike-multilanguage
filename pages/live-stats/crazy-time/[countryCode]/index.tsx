...
import styled from "styled-components";
import {
  crazyTimeSymbolToFilterOption,
  Spin,
} from "../../../../data/models/Spin";
import axios from "axios";
import { CrazyTimeSymbolStat } from "../../../../data/models/CrazyTimeSymbolStat";
import CrazyTimeStatCard from "../../../../components/Cards/CrazyTimeStatCard";
import { CrazyTimeTable } from "../../../../components/CrazyTimeLiveStats/CrazyTimeTable";
...
const index: FunctionComponent<Props> = ({
  ...
  const lazyUpdateInitialData = async () => {
    const update = await axios.get(
      `https://casinowizard.topadscron.it/crazy-time?hours=24`,
    );
    ...
    setFilteredRows(
      rows.filter((r) =>
        selectedFilters.includes(
          crazyTimeSymbolToFilterOption(r.spinResultSymbol),
        )
      ),
    );
  }, [selectedFilters]);
  ...
  const [totalSpinsInTimeFrame, setTotalSpinsInTimeFrame] = useState(
    _stats.totalSpins,
  );
    ...
    <Fragment>
      ...
      >
        <Head>
          ...
          <link
            rel="canonical"
            href={`https://spikeslot.com/live-stats/crazy-time/${contextCountry}`}
          />
          <meta name="description" content={_pageContent.seo.seoDescription}>
          </meta>

          {/* <!-- Google / Search Engine Tags --> */}
          <meta
            itemProp="name"
            content={_pageContent.seo.seoTitle ||
              "SPIKE Slot | Il Blog n.1 in Italia su Slot Machines e Gioco D'azzardo"}
          />
          <meta
        ...
        <BodyContainer>
          <BlockingOverlay
            redirectLink="/live-stats/crazy-time/it"
          ...
          >
            ...
            >
              {_bonuses &&
                _bonuses.map((b) => <BonusStripe key={b.name} bonus={b} />)}
            </Paper>
...
export const getServerSideProps = async ({ query, req, res }) => {
  ...
  const { data: pageData } = await axios.get(
    "https://casinowizard.topadscron.it/crazy-time?hours=3",
  );
  ...
  if (countryCode === "it") {
    ...
    PAGE_BONUSES.forEach((name) =>
      orderedBonusList.push(
        bonuses.data.data.bonuses.find((it) => it.name === name),
      )
    );
  } else {
    const bonuses = await aquaClient.query({
    ...
    bonuses.data.data.homes[0].bonuses.bonus.forEach((b) =>
      orderedBonusList.push(b.bonus)
    );
  }
  ...
  const bonusRemapping = {
    ...
    LeoVegas:
      "https://ntrfr.leovegas.com/redirect.aspx?pid=3704489&lpid=1757&bid=19140",
    "888 Casino": "https://ic.aff-handler.com/c/43431?sr=1868828",
    StarCasinò:
      "https://record.starcasino.it/_SEA3QA6bJTNXl890vMAfUGNd7ZgqdRLk/131/",
    ...
    WinCasino:
      "https://www.wincasinopromo.it/?=registration&mp=cd6cb4e9-42cc-4d51-bc95-46bbb80844a2",
...
export const orderStats = (
  arr: any[],
) => [arr[4], arr[5], arr[6], arr[7], arr[1], arr[2], arr[3], arr[0]];
