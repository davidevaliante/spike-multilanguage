...
app.prepare().then(() => {
  ...
  server.get("/videos/:slug/:countryCode", (req, res) => {
    ...
    if (countryCode !== "it") {
      if (oldUrls.includes(slug)) {
        res.set(
          "location",
          `https://spikeslot.com/videos/${newUrls[oldUrls.indexOf(slug)]}/it`
        );
        res.status(301).send();
      }
  ...
  server.get("/slot-gratis/:slotName/:requestedCountry", async (req, res) => {
    ...
    try {
      const findOldSlot = await axios.get(
        `http://localhost:3000/api/getoldslot?name=${legacySlug}`
      );

      if (findOldSlot.data && findOldSlot.data.slug) {
        res.set(
          "location",
          `https://spikeslot.com/slot/${findOldSlot.data.slug}/it`
        );
        res.status(301).send();
      }
    } catch (error) {
